...
export default function LoginPage() {
  ...
  useEffect(() => {
    supabase.auth.getSession().then(({ data }) => {
      ...
      setLoading(false);
      // If already signed in, redirect to captions
      if (data.session) {
          ...
          <div className="mt-12 rounded-2xl border border-neutral-200 bg-white p-8 shadow-sm">
            <div className="text-center">
              ...
              <button
                ...
                disabled={signingIn}
                className="mt-6 inline-flex cursor-pointer items-center rounded-xl bg-black px-6 py-3 text-sm font-medium text-white transition hover:bg-gray-800 disabled:opacity-50 disabled:cursor-not-allowed"
              >
                <svg
              ...
              <div className="mt-6">
                <Link
                  href="/"
                  className="cursor-pointer text-sm text-neutral-600 transition hover:text-neutral-900"
                >
                  Back to home
                </Link>
              </div>
